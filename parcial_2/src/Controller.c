...
int controller_loadFromText(char* path , LinkedList* pArrayList)
{
  ...
  int exito = 0;
	  ...
	  if(path != NULL && pArrayList != NULL)
	  {
		  ...
		  if(pFile != NULL)
		  {
			  parser_PassengerFromText(pFile, pArrayList);//pide pArchivo y la linkedList
			  exito = 1;
		  }
		  else
		  {
...
int controller_loadFromBinary(char* path , LinkedList* pArrayList)
{
	...
	if(path != NULL && pArrayList != NULL)
	{
		...
		if(pFile != NULL)
		{
			parser_PassengerFromBinary(pFile,pArrayList);
			exito = 1;
		}
		else
		{
...
int controller_ListPassenger(LinkedList* pArrayList)
{
	...
	{
		p = (Passenger*) ll_get(pArrayList,i);
 ...
 * int controller_Listar(LinkedList* pArrayList)
{
	...
	ll_sort(pArrayList, Generico_sortName, 1);

	for(i=0;i<cantidad;i++)
	{
		p = (Empleado*) ll_get(pArrayList,i);
		Generico_printOne(p);
...
int controller_sortPassenger(LinkedList* pArrayList)
{
    ...
    switch(option)
    {
      case 1:
          ...
          ll_sort(pArrayList, Passenger_sortName, orden);
      ...
      case 2:
    	  ...
    	  ll_sort(pArrayList, Passenger_sortPrice, orden);
...
int controller_saveAsText(char* path , LinkedList* pArrayList)
{
	...
	if(pFile != NULL)
	{
      ...
      {
    	  unPasajero = ll_get(pArrayList, i);
      ...
      idMax = Passenger_buscarIdMax(pArrayList);//busca el mayor id del archivo
...
int controller_saveAsBinary(char* path , LinkedList* pArrayList)
{
	...
	if(pFile != NULL)
	{
		...
		for(i=0;i<tam;i++)
		{
			unPasajero = (Passenger*)ll_get(pArrayList, i);
...
int controller_editPassenger(LinkedList* pArrayList)
{
	...
	controller_ListPassenger(pArrayList);

	getInt("\nIngrese en id del pasajero a modificar: ", &id, 1, tam);

	for(i=0;i<tam;i++)
	{
		auxPasajero = ll_get(pArrayList, i);//devuelve una posicion
